...
export default function CustomerList() {
  ...
  const editUser = (item, index) => {
    ...
    window.location = `/admin-customerupdate/edit?edit=true&cid=${item.cid}`;
  }

  const deleteuser = async (item, index) => {

    const url = `http://localhost:9099/admin/delCustomer/${item.cid}`;
  ...
  const getdata = async () => {
    const url = "http://localhost:9099/admin/customer-list";
